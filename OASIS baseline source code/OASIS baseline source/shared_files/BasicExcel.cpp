/*============================================================//
The BasicExcel package (BasicExcel.cpp and BasicExcel.hpp) were 
not created under the ALSAM3024 project and do not fall under 
that project's BSD 2-clause license.

This package was obtained from 
https://www.codeproject.com/Articles/13852/BasicExcel-A-Class-to-Read-and-Write-to-Microsoft
which indicates there is no specific license attached to it.
As such, the code has been utilized as open source with attribution.
...
/*============================================================//
This file contains functions to read and write to XML-style Excel
files, identified by .xls extension (rather than the current .xlsx)

The current project uses only the read function and none of the 
file creation or write functions

Note several major limitations of this package:
	- only handles .xls, not .xlsx format
	- crashes if any field (including Notes) contain >255 characters
	- does not properly read fields containing functions.  All input fields must be hard coded
//============================================================*/
...
int GetRKValueFromDouble(double value)
{
	...
	// which had the effect of adding 1 to rkValue IF isMultiplied==True and rkValue was even
	if (isMultiplied & (rkValue % 2 == 0))
	{
		rkValue += 1;
	}
	return rkValue;
...
BasicExcelWorksheet* BasicExcel::AddWorksheet(int sheetIndex)
{
	...
	{
		char sname[50];
		sprintf(sname, "Sheet%zu", sheetNo++);
